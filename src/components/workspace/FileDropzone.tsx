...
export function FileDropzone({ files, onChange, max = 10 }: { files: File[]; onChange: (f: File[]) => void; max?: number }) {
  ...
  return (
    <div>
      ...
      >
        <p className="text-[0.8125rem] text-bone-200">Drag files here or <button type="button" onClick={() => input.current?.click()} className="font-medium text-forge-300 hover:text-forge-400">browse</button></p>
        <p className="mt-1 text-[0.6875rem] text-bone-600">PNG, JPG, WEBP, SVG, PDF, DOC, XLS, TXT, ZIP · up to 25 MB each · {max} files</p>
        <input ref={input} id={id} type="file" multiple accept={ACCEPT} className="sr-only" onChange={(e) => { add(e.target.files); e.target.value = ""; }} aria-label="Choose files" />
      </div>
      {files.length > 0 && (
