    ...
    <ol className="relative ml-3 border-l border-line">
      {items.map((a) => (
        <li key={a.id} className={compact ? "relative pb-3 pl-5 last:pb-0" : "relative pb-4 pl-5 last:pb-0"}>
          ...
          <p className="text-[0.8125rem] text-bone-200"><span className="inline-flex items-center gap-1.5 font-medium text-bone-50"><Avatar name={a.actorName ?? "System"} image={a.actorImage} size={16} />{a.actorName ?? "System"}</span> {a.href ? <Link href={`/${area}${a.href}`} className="hover:text-forge-300">{a.summary}</Link> : a.summary}</p>
