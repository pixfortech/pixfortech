...
type Props = {
  ...
  children?: ReactNode;
  compact?: boolean;
  /** Decorative scene shown beside the title on large screens, below it on small ones. */
  art?: ReactNode;
};

/** Inner-page opener with the same tone as the home hero, minus the scene. */
export function PageHeader({ eyebrow, title, lead, children, compact = false, art }: Props) {
  ...
  return (
    <header className="relative overflow-hidden border-b border-line">
      ...
      <Container className={compact ? "relative pt-32 pb-12 sm:pt-40 sm:pb-16" : "relative pt-32 pb-16 sm:pt-44 sm:pb-24"}>
        <div className={art ? "grid gap-10 lg:grid-cols-12 lg:items-end" : undefined}>
          <div className={art ? "min-w-0 lg:col-span-8" : undefined}>
            <Rise>
          ...
          {art ? <Rise delay={0.35} className="min-w-0 lg:col-span-4 lg:justify-self-end">{art}</Rise> : null}
        </div>
      </Container>
    </header>
  );
