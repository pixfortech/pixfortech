...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ next?: string; error?: string; verified?: string }> }) {
  ...
  return (
    <AuthCard title="Sign in" lead="Your projects, requests and files, in one place." footer={<>Trouble signing in? <Link href="/forgot-password" className="link-line text-bone-50">Reset your password</Link></>}>
      {verified && <p className="mb-4 rounded-md border border-[#7ed0a2]/40 bg-[#7ed0a2]/10 px-3 py-2 text-[0.8125rem] text-[#9fe0bb]">Email verified. You can sign in now.</p>}
