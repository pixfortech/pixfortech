...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      ...
      <PageTitle title={<span className="flex items-center gap-3"><ProjectMark theme={client.pixelTheme} size={26} />{client.name}</span>} description={[client.industry, client.website].filter(Boolean).join(" · ") || "No details yet."} actions={<>{canManageClients(user) && <ModalButton label="Edit" title="Edit client" variant="secondary"><ClientForm client={client} /></ModalButton>}<AppButton href="/admin/projects/new">New project</AppButton></>} />
      <div className="grid gap-6 lg:grid-cols-12">
        ...
        <Card className="lg:col-span-4">
          <CardHeader title="People" description="Client admins see every project of theirs; members only those they are added to." action={canManageClientUsers(user) ? <ModalButton label="Invite" title="Invite a client user" size="sm" variant="secondary"><InviteForm organisations={[]} defaultOrganisationId={client.id} roles={["client_member", "client_admin"]} /></ModalButton> : undefined} />
