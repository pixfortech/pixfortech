...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div>
      ...
      <PageTitle title={t.title} description={<span className="flex flex-wrap items-center gap-2"><Badge tone={statusTone(t.status)}>{humanise(t.status)}</Badge><Badge tone={priorityTone(t.priority)}>{t.priority}</Badge>{!t.clientVisible && <Badge tone="internal">Internal</Badge>}{t.dueDate && <span className="text-bone-400">Due {fmtDate(t.dueDate, true)}</span>}</span>} actions={<ModalButton label="Edit task" title="Edit task" variant="secondary">{(done) => <TaskForm people={team.map((m) => ({ id: m.id, name: m.name }))} milestones={milestones.map((m) => ({ id: m.id, title: m.title }))} task={t} onDone={done} />}</ModalButton>} />
