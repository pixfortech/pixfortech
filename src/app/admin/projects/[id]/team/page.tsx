...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <ProjectPage user={user} id={id} area="admin" tab="/team">{(project) => (<Card><CardHeader title="Who is on this project" description="Staff members see the project in their lists; client members get portal access to it. Client admins always see their organisation's projects." /><div className="px-5 pb-5"><MembersForm projectId={project.id} people={assignableUsers(user, project.organisationId)} current={project.members.map((m) => ({ id: m.id, projectRole: m.projectRole }))} /></div></Card>)}</ProjectPage>;
}
