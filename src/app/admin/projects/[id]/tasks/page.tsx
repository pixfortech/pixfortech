...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <ProjectPage user={user} id={id} area="admin" tab="/tasks">{async (project) => {
    ...
    return (<div><div className="mb-4 flex justify-end"><ModalButton label="New task" title="New task" size="sm">{(done) => <TaskForm projectId={project.id} people={people} milestones={milestones.map((m) => ({ id: m.id, title: m.title }))} onDone={done} />}</ModalButton></div><TaskBoard tasks={tasks} /></div>);
  }}</ProjectPage>;
}
