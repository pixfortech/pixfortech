...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <ProjectPage user={user} id={id} area="admin" tab="/approvals">{async (project) => {
    const [list, milestones, requests] = await Promise.all([(await listApprovals(user, { projectId: project.id })), (await listMilestones(user, project.id)), (await listRequests(user, { projectId: project.id, open: true }))]);
