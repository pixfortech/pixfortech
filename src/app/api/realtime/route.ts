...
import { clientEvent } from "@/server/realtime/client-event";
...
export async function GET(req: Request) {
  ...
  return Response.json({ events: result.events.map(clientEvent), cursor: now, unread, resync: result.overflow }, { headers: { "cache-control": "private, no-store" } });
}
