...
export async function listClients(actor: Actor) {
  ...
  const orgs = (await db.select().from(schema.organisations).where(and(eq(schema.organisations.kind, "client"), isNull(schema.organisations.deletedAt))).orderBy(asc(schema.organisations.name)));
  return (await Promise.all(orgs.map(async (o) => ({
    ...o,
    projects: (await db.select({ n: sql<number>`count(*)::int` }).from(schema.projects).where(and(eq(schema.projects.organisationId, o.id), isNull(schema.projects.deletedAt))).limit(1))[0]?.n ?? 0,
    users: (await db.select({ n: sql<number>`count(*)::int` }).from(schema.users).where(eq(schema.users.organisationId, o.id)).limit(1))[0]?.n ?? 0,
  }))));
}
...
export async function getClient(actor: Actor, id: string) {
  ...
  const org = (await db.select().from(schema.organisations).where(and(eq(schema.organisations.id, id), isNull(schema.organisations.deletedAt))).limit(1))[0];
  if (!org) return null;
  const users = (await db.select({ id: schema.users.id, name: schema.users.name, email: schema.users.email, role: schema.users.role, image: schema.users.image, title: schema.users.title, disabled: schema.users.disabled, emailVerified: schema.users.emailVerified, createdAt: schema.users.createdAt }).from(schema.users).where(eq(schema.users.organisationId, id)).orderBy(asc(schema.users.name)));
  const projects = (await db.select().from(schema.projects).where(and(eq(schema.projects.organisationId, id), isNull(schema.projects.deletedAt))).orderBy(desc(schema.projects.updatedAt)));
  return { ...org, users, projects };
}

export async function createClient(actor: Actor, input: { name: string; website?: string; industry?: string; notes?: string; pixelTheme?: string | null }) {
  ...
  (await db.insert(schema.organisations).values({ id, name: input.name, slug, kind: "client", website: input.website ?? null, industry: input.industry ?? null, notes: input.notes ?? null, pixelTheme: input.pixelTheme ?? null }));
  (await recordAudit({ actorId: actor.id, action: "client.create", targetType: "organisation", targetId: id, metadata: { name: input.name } }));
  return id;
}

export async function updateClient(actor: Actor, id: string, patch: { name?: string; website?: string | null; industry?: string | null; notes?: string | null; pixelTheme?: string | null }) {
  if (!canManageClients(actor)) throw new AuthError(403, "Only admins and project managers can edit clients.");
  (await db.update(schema.organisations).set({ ...patch, updatedAt: new Date() }).where(eq(schema.organisations.id, id)));
  (await recordAudit({ actorId: actor.id, action: "client.update", targetType: "organisation", targetId: id, metadata: patch }));
}

export async function listTeam(actor: Actor) {
  if (!isStaff(actor)) throw new AuthError(403, "Staff only.");
  const staff = (await db.select({ id: schema.users.id, name: schema.users.name, email: schema.users.email, role: schema.users.role, image: schema.users.image, title: schema.users.title, disabled: schema.users.disabled, createdAt: schema.users.createdAt })
    .from(schema.users).where(or(...STAFF_ROLES.map((r) => eq(schema.users.role, r)))).orderBy(asc(schema.users.name)));
  return (await Promise.all(staff.map(async (u) => ({
    ...u,
    openTasks: (await db.select({ n: sql<number>`count(*)::int` }).from(schema.tasks).where(and(eq(schema.tasks.assigneeId, u.id), sql`${schema.tasks.status} != 'done'`)).limit(1))[0]?.n ?? 0,
    openRequests: (await db.select({ n: sql<number>`count(*)::int` }).from(schema.requests).where(and(eq(schema.requests.assigneeId, u.id), sql`${schema.requests.status} not in ('completed','closed','rejected','cancelled')`)).limit(1))[0]?.n ?? 0,
    managing: (await db.select({ n: sql<number>`count(*)::int` }).from(schema.projects).where(and(eq(schema.projects.managerId, u.id), isNull(schema.projects.deletedAt), sql`${schema.projects.status} not in ('completed','on_hold')`)).limit(1))[0]?.n ?? 0,
  }))));
}

/** Staff-side listing of people for assignment pickers: all staff plus the client users of a given organisation. */
export async function assignableUsers(actor: Actor, organisationId?: string) {
  ...
  return (await db.select({ id: schema.users.id, name: schema.users.name, role: schema.users.role, image: schema.users.image, organisationId: schema.users.organisationId }).from(schema.users).where(and(conds, eq(schema.users.disabled, false))).orderBy(asc(schema.users.name)));
}
...
export async function inviteUser(actor: Actor, input: { name: string; email: string; role: Role; organisationId: string | null; title?: string }, ip?: string | null) {
  ...
  (await recordAudit({ actorId: actor.id, action: "user.invite", targetType: "user", targetId: userId, metadata: { email: input.email, role: input.role, organisationId: input.organisationId, emailed }, ip }));
  return { id: userId, emailed };
}

export async function updateUser(actor: Actor, id: string, patch: { role?: Role; disabled?: boolean; title?: string | null; organisationId?: string | null }, ip?: string | null) {
  const target = (await db.select().from(schema.users).where(eq(schema.users.id, id)).limit(1))[0];
  ...
  (await db.update(schema.users).set({ ...patch, updatedAt: new Date() }).where(eq(schema.users.id, id)));
  if (patch.disabled) (await db.delete(schema.sessions).where(eq(schema.sessions.userId, id)));
  (await recordAudit({ actorId: actor.id, action: "user.update", targetType: "user", targetId: id, metadata: patch, ip }));
}

export async function updateProfile(actor: Actor, patch: { name?: string; title?: string | null; timezone?: string | null }) {
  (await db.update(schema.users).set({ ...patch, updatedAt: new Date() }).where(eq(schema.users.id, actor.id)));
}
...
export async function search(actor: Actor, q: string) {
  ...
  const projects = (await db.select({ id: schema.projects.id, code: schema.projects.code, title: schema.projects.title, status: schema.projects.status }).from(schema.projects).where(and(sql`${schema.projects.id} in ${within}`, isNull(schema.projects.deletedAt), or(ilike(schema.projects.title, term), ilike(schema.projects.code, term)))).limit(6));
  const taskConds = [sql`${schema.tasks.projectId} in ${within}`, or(ilike(schema.tasks.title, term), ilike(schema.tasks.key, term))];
  ...
  const messages = (await db.select({ id: schema.messages.id, body: schema.messages.body, conversationId: schema.messages.conversationId, requestId: schema.conversations.requestId, projectId: schema.conversations.projectId })
    .from(schema.messages).innerJoin(schema.conversations, eq(schema.conversations.id, schema.messages.conversationId)).where(and(...convConds, isNull(schema.messages.deletedAt), ilike(schema.messages.body, term))).limit(5));
