...
export async function listTasks(actor: Actor, filter: { projectId?: string; assigneeId?: string; status?: string; priority?: string; due?: "today" | "overdue" | "week"; requestId?: string; milestoneId?: string } = {}) {
  ...
  return (await db.select(taskSelect).from(schema.tasks)
    ...
    .where(and(...conds)).orderBy(asc(schema.tasks.order), desc(schema.tasks.updatedAt)));
}

export async function getTask(actor: Actor, id: string) {
  const t = (await db.select(taskSelect).from(schema.tasks).leftJoin(schema.users, eq(schema.users.id, schema.tasks.assigneeId)).innerJoin(schema.projects, eq(schema.projects.id, schema.tasks.projectId)).where(eq(schema.tasks.id, id)).limit(1))[0];
  if (!t) return null;
  const project = await requireProject(actor, t.projectId);
  if (!isStaff(actor) && !t.clientVisible) return null;
  const comments = (await db.select({ id: schema.taskComments.id, body: schema.taskComments.body, internal: schema.taskComments.internal, createdAt: schema.taskComments.createdAt, authorName: schema.users.name, authorImage: schema.users.image })
...
export async function createTask(actor: Actor, projectId: string, input: TaskInput) {
  ...
  const id = uid();
  const n = (await nextNumber(`task:${project.code}`));
  ...
  (await recordActivity({ organisationId: project.organisationId, projectId, actorId: actor.id, kind: "task.created", summary: `created task ${key} “${input.title}”`, targetType: "task", targetId: id, href: `/tasks/${id}`, internal: !(input.clientVisible ?? false) }));
  ...
  (await publish({ type: "task.created", audience: { projectIds: [projectId] }, payload: { id, projectId } }));
  return { id, key };
}
...
export async function updateTask(actor: Actor, id: string, patch: Partial<TaskInput>) {
  ...
  const t = (await db.select().from(schema.tasks).where(eq(schema.tasks.id, id)).limit(1))[0];
  ...
  if (patch.status && patch.status !== t.status) {
    (await recordActivity({ organisationId: project.organisationId, projectId: t.projectId, actorId: actor.id, kind: "task.status", summary: `moved ${t.key} to ${TASK_STATUS_LABELS[patch.status]}`, targetType: "task", targetId: id, href: `/tasks/${id}`, internal: !t.clientVisible }));
    if (patch.status === "done") await notify({ recipientIds: (await projectStaffIds(t.projectId, project.managerId)), category: "task", title: `${t.key} completed`, body: t.title, href: `/tasks/${id}`, projectId: t.projectId, actorId: actor.id });
  }
  ...
  (await publish({ type: "task.updated", audience: { projectIds: [t.projectId] }, payload: { id, projectId: t.projectId, status: patch.status ?? t.status } }));
}

export async function addTaskComment(actor: Actor, taskId: string, body: string, internal: boolean) {
  const t = (await db.select().from(schema.tasks).where(eq(schema.tasks.id, taskId)).limit(1))[0];
  if (!t) throw new AuthError(403, "Task not found.");
  const project = await requireProject(actor, t.projectId);
  if (!isStaff(actor) && (!t.clientVisible || internal)) throw new AuthError(403, "Not allowed.");
  const id = uid();
  (await db.insert(schema.taskComments).values({ id, taskId, authorId: actor.id, body, internal: isStaff(actor) ? internal : false }));
  (await recordActivity({ organisationId: project.organisationId, projectId: t.projectId, actorId: actor.id, kind: "task.comment", summary: `commented on ${t.key}`, targetType: "task", targetId: taskId, href: `/tasks/${taskId}`, internal }));
  const recipients = new Set((await projectStaffIds(t.projectId, project.managerId)));
  ...
  (await publish({ type: "task.updated", audience: { projectIds: [t.projectId] }, payload: { id: taskId, projectId: t.projectId } }));
  return id;
}
