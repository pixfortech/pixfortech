...
export async function listRequests(actor: Actor, filter: { projectId?: string; status?: string; type?: string; priority?: string; assigneeId?: string; organisationId?: string; open?: boolean } = {}) {
  ...
  const rows = db.select(select).from(schema.requests).innerJoin(schema.projects, eq(schema.projects.id, schema.requests.projectId)).where(and(...conds)).orderBy(desc(schema.requests.updatedAt)).all();
  const people = usersByIds([...new Set(rows.flatMap((r) => [r.requesterId, r.assigneeId].filter((x): x is string => Boolean(x))))]);
...
export async function getRequest(actor: Actor, id: string) {
  const full = db.select().from(schema.requests).where(eq(schema.requests.id, id)).get();
  if (!full) return null;
  ...
  const comments = db.select({ id: schema.requestComments.id, body: schema.requestComments.body, internal: schema.requestComments.internal, createdAt: schema.requestComments.createdAt, authorId: schema.requestComments.authorId, authorName: schema.users.name, authorImage: schema.users.image, authorRole: schema.users.role })
    ...
    .orderBy(asc(schema.requestComments.createdAt)).all();
  ...
  const files = db.select().from(schema.files).where(and(...fileConds)).all();
  const taskConds = [eq(schema.tasks.requestId, id)];
  if (!staff) taskConds.push(eq(schema.tasks.clientVisible, true));
  const tasks = db.select({ id: schema.tasks.id, key: schema.tasks.key, title: schema.tasks.title, status: schema.tasks.status, assigneeId: schema.tasks.assigneeId }).from(schema.tasks).where(and(...taskConds)).all();
  ...
  const history = db.select({ id: schema.activityEvents.id, kind: schema.activityEvents.kind, summary: schema.activityEvents.summary, createdAt: schema.activityEvents.createdAt, internal: schema.activityEvents.internal, actorName: schema.users.name })
    .from(schema.activityEvents).leftJoin(schema.users, eq(schema.users.id, schema.activityEvents.actorId)).where(and(...histConds)).orderBy(asc(schema.activityEvents.createdAt)).all();
  const approvals = db.select().from(schema.approvals).where(eq(schema.approvals.requestId, id)).all();
  const people = usersByIds([full.requesterId, full.assigneeId].filter((x): x is string => Boolean(x)));
  const thread = db.select({ id: schema.conversations.id }).from(schema.conversations).where(eq(schema.conversations.requestId, id)).get();
  return {
...
export async function createRequest(actor: Actor, input: RequestInput) {
  ...
  const id = uid();
  const number = nextNumber("request");
  ...
  db.insert(schema.conversations).values({ id: uid(), organisationId: project.organisationId, projectId: project.id, requestId: id, title: `${ref} · ${input.title}`, internal: false }).run();
  recordActivity({ organisationId: project.organisationId, projectId: project.id, actorId: actor.id, kind: "request.created", summary: `submitted ${ref} “${input.title}”`, targetType: "request", targetId: id, href: `/requests/${id}` });
  await notify({ recipientIds: projectStaffIds(project.id, project.managerId), category: "request", title: `New change request · ${ref}`, body: `${project.title}: “${input.title}”`, href: `/requests/${id}`, projectId: project.id, actorId: actor.id });
  publish({ type: "request.created", audience: { projectIds: [project.id], staff: true }, payload: { id, ref, projectId: project.id } });
  return { id, ref };
}
...
export async function transitionRequest(actor: Actor, id: string, status: RequestStatus, note?: string) {
  ...
  db.update(schema.requests).set({ status, updatedAt: new Date(), closedAt: closed ? new Date() : null }).where(eq(schema.requests.id, id)).run();
  const ref = requestCode(r.number);
  recordActivity({ organisationId: project.organisationId, projectId: project.id, actorId: actor.id, kind: "request.status", summary: `moved ${ref} to ${REQUEST_STATUS_LABELS[status]}${note ? ` · ${note}` : ""}`, targetType: "request", targetId: id, href: `/requests/${id}` });
  const recipients = isStaff(actor) ? [...projectClientIds(project.id, project.organisationId), r.requesterId] : projectStaffIds(project.id, project.managerId);
  ...
  publish({ type: "request.updated", audience: { projectIds: [project.id], staff: true }, payload: { id, ref, status, projectId: project.id } });
}
...
export async function assignRequest(actor: Actor, id: string, assigneeId: string | null, estimate?: string | null, estimatedCompletion?: Date | null) {
  ...
  if (!r) throw new AuthError(403, "Request not found.");
  const project = await requireProject(actor, r.projectId);
  db.update(schema.requests).set({ assigneeId, estimate: estimate === undefined ? r.estimate : estimate, estimatedCompletion: estimatedCompletion === undefined ? r.estimatedCompletion : estimatedCompletion, updatedAt: new Date() }).where(eq(schema.requests.id, id)).run();
  const ref = requestCode(r.number);
  if (assigneeId && assigneeId !== r.assigneeId) {
    recordActivity({ organisationId: project.organisationId, projectId: project.id, actorId: actor.id, kind: "request.assigned", summary: `assigned ${ref}`, targetType: "request", targetId: id, href: `/requests/${id}`, internal: true });
  ...
  if (estimate !== undefined || estimatedCompletion !== undefined) {
    recordActivity({ organisationId: project.organisationId, projectId: project.id, actorId: actor.id, kind: "request.estimated", summary: `updated the estimate for ${ref}`, targetType: "request", targetId: id, href: `/requests/${id}` });
  }
  publish({ type: "request.updated", audience: { projectIds: [project.id], staff: true }, payload: { id, ref, projectId: project.id } });
}

function actorNameFor(id: string): string {
  return db.select({ name: schema.users.name }).from(schema.users).where(eq(schema.users.id, id)).get()?.name ?? "Someone";
}
...
export async function addRequestComment(actor: Actor, id: string, body: string, internal: boolean) {
  ...
  const cid = uid();
  db.insert(schema.requestComments).values({ id: cid, requestId: id, authorId: actor.id, body, internal: isInternal }).run();
  db.update(schema.requests).set({ updatedAt: new Date() }).where(eq(schema.requests.id, id)).run();
  const ref = requestCode(r.number);
  recordActivity({ organisationId: project.organisationId, projectId: project.id, actorId: actor.id, kind: "request.comment", summary: isInternal ? `added an internal note to ${ref}` : `commented on ${ref}`, targetType: "request", targetId: id, href: `/requests/${id}`, internal: isInternal });
  const recipients = isInternal ? projectStaffIds(project.id, project.managerId) : [...projectStaffIds(project.id, project.managerId), ...projectClientIds(project.id, project.organisationId), r.requesterId];
  await notify({ recipientIds: recipients, category: "request", title: `${actorNameFor(actor.id)} commented on ${ref}`, body: body.slice(0, 140), href: `/requests/${id}`, projectId: project.id, actorId: actor.id });
  publish({ type: "request.updated", audience: isInternal ? { staff: true } : { projectIds: [project.id], staff: true }, payload: { id, ref, projectId: project.id, comment: true } });
  return cid;
}
