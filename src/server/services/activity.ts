...
export function recordActivity(input: {
  ...
  if (input.projectId) publish({ type: "project.updated", audience: { projectIds: [input.projectId] }, payload: { projectId: input.projectId, kind: input.kind } });
  return id;
}

export function recordAudit(input: { actorId: string | null; action: string; targetType: string; targetId?: string; metadata?: Record<string, unknown>; ip?: string | null }) {
...
export function listActivity(actor: Actor, opts: { projectIds: string[]; limit?: number; projectId?: string }) {
  ...
  return db.select({
    ...
    .where(where).orderBy(desc(schema.activityEvents.createdAt)).limit(opts.limit ?? 30).all();
}
...
export function listAudit(limit = 100) {
  ...
  }).from(schema.auditEvents).leftJoin(schema.users, eq(schema.users.id, schema.auditEvents.actorId)).orderBy(desc(schema.auditEvents.createdAt)).limit(limit).all();
}
