...
// 5. Kanban drag and drop persists
{
  ...
  const card = todo.locator("li[draggable]").first();
  const key = (await card.textContent())?.match(/[A-Z]+-\d+/)?.[0];
  ...
  const moved = key ? (await pm.page.getByRole("region", { name: "In progress" }).textContent())?.includes(key) : false;
  ok("kanban drag persists after reload", !!moved && after === beforeCount + 1, `${key} · ${beforeCount} → ${after}`);
