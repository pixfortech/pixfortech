...
// 5. Kanban drag and drop persists
{
  ...
  await pm.page.locator('li[draggable="true"]').first().waitFor();
  const sourceLabel = await pm.page.getByRole("region", { name: "To do", exact: true }).locator("li[draggable]").count() ? "To do" : "In progress";
  const targetLabel = sourceLabel === "To do" ? "In progress" : "To do";
  const todo = pm.page.getByRole("region", { name: sourceLabel, exact: true });
  const inProgress = pm.page.getByRole("region", { name: targetLabel, exact: true });
  ...
  const moved = href ? await inProgress.locator(`a[href='${href}']`).count() === 1 : false;
