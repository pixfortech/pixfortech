...
**Owner bootstrap**: `npm run admin:check` and `npm run admin:bootstrap` (add `--require-verification --app=<origin>` to gate the first sign-in on the emailed link). **Email delivery**: `npm run email:check -- --to=<address>` sends a probe through Resend and reports its delivery event; `--status=<id>` looks one up; see `docs/DEPLOYMENT.md`.
