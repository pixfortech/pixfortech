...
**The forge gate** (sign-in screens): every auth page shares a stage beside the form where PiP keeps watch. Forms report plain events (a field focused, typing started, a failed or successful attempt) to a small store in `src/pixel/auth/store.ts`; a pure reducer in `logic.ts` turns them into one of sixteen faces (`emotions.ts`) and, occasionally, a line from the `auth*` categories through the mascot's non-repeating history. Password entry covers his eyes after a brief peek and the face never depends on what is typed; a shown password keeps him turned away until it is hidden. Reduced motion keeps the faces and drops the movement. Sign-out lands on `/login?signedout=1` where he waves.

**PiP's bench** (home page, beside the first section heading): a canvas workstation where PiP gathers loose pixels, builds one of fourteen small pieces (`src/pixel/bench/objects.ts`), checks it against a ruler and snap lines, corrects the pixel that landed a cell out, and places the finished piece on its grid. Rotation is a shuffled non-repeating cycle (`cycle.ts`), the machine is pure and time-stepped (`bench.ts`, unit-tested), rendering is one 2D canvas (`draw.ts`), and the component gates itself on visibility, page settle and the tab being shown. Reduced motion shows a finished composition with a blink. Lines come from the `precisionBuild`, `precisionInspect`, `precisionComplete` and `benchPoke` categories through the store's history, so nothing repeats until the category is spent.
